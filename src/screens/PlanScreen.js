...
function PlanScreen() {
    ...
    const loadCheckout = async (priceId) => {
         const docRef = await db.collection('customers').doc(
            user.uid).collection("checkout_sessions").add({
                price: priceId,
  ...
  return (
    <div className='planScreen'>
        ...
        {Object.entries(products).map(([productId, productData]) => {
           ...
           return(
                <div key={productId} className={'${isCurrentPackage && "plansScreen_plan--disabled"} planScreen_plan'}>
